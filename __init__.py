...
GitHub: https://github.com/yourname/comfyui-vlm-dense-caption
...
- component_mod: 修改组件
  修改鞋带系统（扁鞋带→圆鞋带、鞋带→魔术贴、弹性鞋口等）、鞋舌样式、鞋口造型、侧边支撑、拉链(zipper)等
  示例: "Replace the traditional flat laces with an elastic speed-lacing system with a toggle closure"

- accessory_add: 添加/修改配饰
  可选项：蝴蝶结(bow)、钻饰(rhinestones)、五金饰扣(buckle)、珍珠(pearl)、流苏(tassel)、铆钉(rivet)、链条(chain)、丝带(banderole)、亮片(sequin)、星星(star)、花朵(flower)、金属(metal)、宝石(gems)、运动搭扣(sport buckle)
...
- craft_upgrade: 产品工艺升级
  可选项：压纹(embossing)、编织(knit)、刺绣(embroidery)、绗缝(quilted)、粗饰线(heavy stitching)、抽褶(shirring)、填海绵(padded sponge)、印花(printing)、做旧(distressed)、镂空(cutout)
...
- 对于 material_swap 类型：只更换材质，严禁修改原鞋的任何颜色、配色方案或颜色细节，target_caption 中的颜色描述必须与原描述完全一致"""
...
STYLE_MOD_TARGETS = [
    ...
    "stretch boots (弹力靴)",
    "platform shoes (松糕鞋)",
    "fashion casual shoes (时尚休闲鞋)",
    "professional sports shoes (专业运动鞋)",
    "outdoor casual shoes (户外休闲鞋)",
    "fashion sandals (时装凉鞋)",
    "short tube boots (短筒靴)",
    "western boots (西部靴)",
    "knee-high boots (高筒靴)",
    "work shoes (工装鞋)",
    "ballet flats (芭蕾鞋)",
...
class LLMEditInstructionBatch:
    ...
    def generate_batch(
        ...
        for edit_type in chosen:
            ...
            if parsed is None:
                parsed = _fallback_edit(source_caption)

            instructions.append(f"[{edit_type}] {parsed['instruction']}")
            target_captions.append(f"[{edit_type}] {parsed['target_caption']}")
