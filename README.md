...
| `base_url` | STRING | `https://oneapi.newbelle.com.cn` | — | OpenAI 兼容 API 的基础地址。会自动补全 `/v1` 后缀。 |
| `api_key` | STRING | (空) | — | API 密钥。若后端不需要鉴权（如本地 vLLM/Ollama）可留空。 |
| `vl_model` | STRING | `Qwen2.5-VL-32B-Instruct-AWQ` | — | 视觉语言模型名称。需与后端支持的模型名完全一致。 |
| `prompt` | STRING | 鞋类稠密描述 prompt | — | 打标指令文本。**多行输入**，可自定义描述维度和输出格式。 |
| `temperature` | FLOAT | `0.3` | 0.0 ~ 2.0，步长 0.05 | **采样温度**。控制输出的随机性：越低输出越确定/稳定，越高越有创造性。打标场景建议 0.1~0.3。 |
| `max_tokens` | INT | `2000` | 1 ~ 32768，步长 1 | **最大输出 token 数**。限制模型生成的最大长度。稠密描述建议 1500~3000。 |
| `clean_markdown` | BOOLEAN | `true` | — | **是否清理 Markdown 格式**。开启后会移除 `#` 标题、`**` 粗体、`---` 分隔线等 Markdown 标记，并确保以句号结尾。 |
...
> 共享参数：`base_url`、`api_key`、`vl_model`、`prompt`、`temperature`、`max_tokens`、`clean_markdown` 同上。
...
| `edit_type` | 下拉选择 | `material_swap` | 12 种类型 | **编辑类型**（详见下方编辑类型表）。决定修改的维度。 |
| `base_url` | STRING | `https://oneapi.newbelle.com.cn` | — | 同上，OpenAI 兼容 API 地址。 |
| `api_key` | STRING | (空) | — | 同上，API 密钥。 |
| `llm_model` | STRING | `Qwen3-235B-A22B-Instruct-2507-AWQ` | — | **文本 LLM 模型名**。用于生成编辑指令（非 VLM，纯文本模型即可）。 |
| `temperature` | FLOAT | `0.7` | 0.0 ~ 2.0，步长 0.05 | **采样温度**。编辑指令生成需要一定创造性，建议 0.6~0.9。 |
| `max_tokens` | INT | `1500` | 1 ~ 32768，步长 1 | **最大输出 token 数**。 |
| `max_retries` | INT | `2` | 0 ~ 5，步长 1 | **最大重试次数**。当 LLM 返回的 JSON 解析失败时，自动重试的次数。0 表示不重试。 |
| `style_target` | STRING | (空) | — | **款式变换目标**（可选）。仅当 `edit_type=style_mod` 时生效。填入目标品类名，如 `loafers (乐福鞋)`。留空则由模型自由选择。 |
...
除以下参数外，其余参数含义与 **LLM Edit Instruction** 完全相同：

| 参数 | 类型 | 默认值 | 范围 | 说明 |
|------|------|--------|------|------|
| `num_edits` | INT | `3` | 1 ~ 12，步长 1 | **生成编辑指令的数量**。决定最终输出几条不同的编辑。 |
| `fixed_edit_types` | STRING | `style_mod` | — | **固定编辑类型**（逗号分隔）。这些类型一定会出现在结果中。留空则全部随机抽取。如 `style_mod,color_change`。 |
| `enable_style_mod` | BOOLEAN | `true` | — | **是否包含品类变换**。关闭后，随机抽取的编辑类型中将排除 `style_mod`。如果 `fixed_edit_types` 中指定了 `style_mod`，该开关不影响固定列表。 |

> 共享参数：`source_caption`、`base_url`、`api_key`、`llm_model`、`temperature`、`max_tokens`、`max_retries` 同上。
...
| 后端 | `base_url` | 适用模型 |
|------|-----------|----------|
| NewBelle | `https://oneapi.newbelle.com.cn` | Qwen2.5-VL / Qwen3 系列 |
...
| 包 | 最低版本 | 说明 |
|----|----------|------|
| `openai` | ≥ 1.0.0 | OpenAI 兼容客户端（pip 安装） |
